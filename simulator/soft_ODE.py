...
def dyn_GRN(argdict):
    ...
    if _argdict["G0"] is None:
        ...
        idx = 0
        while len(np.nonzero(G0)[0]) < nedges:
            # print(len(np.nonzero(G0)[0]), ngenes)
            target = idx%(ngenes-ntfs) + ntfs
            tf = np.random.choice(ntfs)
            G0[tf,target] = np.random.uniform(low=0, high=1, size=1)
            idx += 1
...
def run_simulator(**setting):
    ...
    mgenes = _setting["mgenes"]
    _setting["m_gene"] = {gene:float(mgenes) for gene in np.arange(_setting["ngenes"])}
    ...
    pseudotime_index = np.random.choice(np.arange(_setting["ntimes"])[50:], _setting["ncells"], replace = True) #replace = False > True (for multiple samples)
    # pseudotime, (ncells,)
    ...
    backbone = _setting["backbone"][pseudotime_index]
    ...
    backbone = backbone[np.argsort(pseudotime)]
